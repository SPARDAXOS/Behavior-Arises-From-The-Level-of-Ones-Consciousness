...
public class DamageDealer : Character
{
    ...
    private float AttackBuffSpellCost = 75.0f;
    private float AttackBuffSpellRate = 12.0f;
    private int AttackBuffSpellTurns = 4;
    private StatusAilments.Buffs AttackBuffSpellType = StatusAilments.Buffs.ATTACK;
    private float SuperDamageRate = 20.0f;
    ...
    private string SuperDamageRateKey = "SuperDamageRate";
    ...
    {
        Portrait = Resources.Load<Sprite>("Sprites/DamageDealerPortrait");
        if (!Portrait)
            Debug.LogError("DamageDealer portrait was not loaded correctly");
    }
    ...
    private void SetupPCHIndex()
    {
        ...
        if (PCHIndex == -1)
        {
            Debug.LogError("PCHIndex for Damage Dealer is -1");
            return;
        }
    }
    ...
    protected override void SetupBlackboardEntries()
    {
        ...
        //Damage Dealer Only - Unique Super
        BB.AddValue<float>(SuperDamageRateKey, SuperDamageRate);
        ...
        BB.AddValue<float>(LowestCostSpellKey, AttackBuffSpellCost); //Depends on individual character
    ...
    public void SetPCScriptReference(PlayersController controller)
    {
        if (!controller)
        {
            Debug.LogError("Null players controller reference sent to Damage Dealer");
        }
        PCScript = controller;
    }
    ...
    public string GetSuperDamageRateKey()
    {
        return SuperDamageRateKey;
    }
